...
def train_one_epoch_adversarial(model, classifier, criterion, data_loader,
                                 optimizer_enc, optimizer_cls, optimizer_adv,
                                 adversary_classifier, criterion_adv, device, epoch,
                                 loss_scaler, max_norm=None, mixup_fn=None,
                                 log_writer=None, args=None):

    model.train()
    classifier.train()
    adversary_classifier.train()
    ...
    for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if len(batch) == 3:
            samples, targets, private_targets = batch
        ...
        # ── Phase 0: defend against AC ──────────────────────────────────
        if phase == 0:
            model.train()
            classifier.eval()
            adversary_classifier.eval()

            with torch.cuda.amp.autocast():
                features  = model.forward_features(samples)
                logits_c  = classifier(features)
                logits_ac = adversary_classifier(features)

                loss_c  = criterion(logits_c, targets)
                loss_ac = criterion_adv(logits_ac, private_targets)
                loss    = loss_c - args.lambda_adv * loss_ac

            optimizer_enc.zero_grad()
            loss_scaler(loss, optimizer_enc, clip_grad=max_norm,
                        parameters=model.parameters(), create_graph=False)

            metric_logger.update(loss_defend=loss.item())
            metric_logger.update(loss_c=loss_c.item())
            metric_logger.update(loss_ac_defend=loss_ac.item())
        ...
        # ── Phase 1: update AC ───────────────────────────────────────────
            ...
            loss_scaler(loss_ac, optimizer_adv, clip_grad=max_norm,
                        parameters=adversary_classifier.parameters(), create_graph=False)

            metric_logger.update(loss_ac_train=loss_ac.item())
        ...
        # ── Phase 2: update C ────────────────────────────────────────────
            ...
            loss_scaler(loss_c, optimizer_cls, clip_grad=max_norm,
                        parameters=classifier.parameters(), create_graph=False)

            metric_logger.update(loss_c_train=loss_c.item())

        metric_logger.update(lr=optimizer_enc.param_groups[-1]["lr"])
